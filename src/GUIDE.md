...
4. **只有 `EXERCISES.md` 裡狀態是 `可用` 的題目可以直接出**。如果狀態是 `不出題` 或找不到該集條目，照 `EXERCISES.md` 開頭的章節預設策略與狀態規則處理。
...
5. 若該集沒有可用題目，依 `EXERCISES.md` 開頭的章節預設策略與狀態規則處理。
6. 如果讀者仍然堅持想練沒有固定題目的指定集數，才可以臨時出 **1 題**。第 1、2 章題目前要明說「這一集題庫沒有固定題目，下面是我依照你目前進度臨時出的練習」。
7. 第 3 章與之後若臨時出題，題目前要明說「第 3 章之後沒有固定題庫，下面是臨時挑戰題；它可能會用到後面才教的內容，你不需要堅持一定要靠自己解出來」。
8. 臨時題目優先小而完整。第 1、2 章臨時題要保守，不要設計成大型綜合題；第 3 章與之後可以出挑戰題，但仍要避免一次塞太多未學工具。
9. 如果無法讀取 `EXERCISES.md`，不要自行出題；請明白告訴讀者你看不到固定題庫，請他補上檔案或改問概念。

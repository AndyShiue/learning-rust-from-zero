...
這份文件是給 AI 用的固定題庫。讀者要練習題時，AI 必須優先從這裡挑題，而不是現場自由發揮。

這份題庫不是「靈感來源」，而是「限制來源」。AI 不能看完本文件後自行加出一堆更進階、更完整、或更像一般 Rust 課程的題目；只能依讀者進度與條目狀態出題。

## 章節預設策略

- 第 1 章、第 2 章預設會安排練習題。若某集標成 `不出題`，代表作者刻意判斷該集不適合出題；AI 回應時必須簡短說明該集的不出題原因，再建議前面最近的 `可用` 題目或請讀者繼續讀。
- 第 3 章與之後不安排固定練習題。這些章節很多內容偏資料建模、語法表達、工程組織或慣例整理，硬放固定題容易變成重寫範例或名詞問答。
- 讀者讀到第 3 章或之後時，若他主動要求題目，AI 可以臨時出題，但必須先說明：這不是固定題庫題目，難度與用法不一定貼齊本書進度，可能會用到後面才教的內容；讀者不需要堅持一定要靠自己解出來。
...
2. 先看 `狀態`。
3. 只有 `可用` 可以直接出題。
4. `不出題` 代表作者刻意不安排固定題；不要直接當成題庫題目來出，回應方式依章節預設策略判斷。
...
- `TODO`：題目尚未填寫；不要假裝有固定題目。
- `不出題`：作者刻意不安排固定題；若條目有「不出題原因」，回應時要簡短說明。

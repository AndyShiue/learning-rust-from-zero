...
- `join!` `poll`s its branches in turn **within the same `Task`**; they do **not** become independent `Task`s and are never moved to other `Thread`s.
...
- Unlike `spawn`: `join!`'s branches don't become independent `Task`s and don't cross `Thread`s — suited to fixed-count, right-here-right-now concurrent I/O.

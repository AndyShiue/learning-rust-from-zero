...
Last episode's server had one defining trait: it spends most of its time **waiting**. Waiting for someone to connect, for data to arrive, for a response to be written out. The time actually spent computing on the CPU is pitifully small.
...
- OS threads are memory-hungry; "one thread per connection" can't sustain huge connection counts — `async` advances masses of work on just a few threads.
...
- `async` provides concurrency; parallelism depends on how many threads the runtime uses.

...
| Chapter 6 | Function pointers, closures, `Fn`/`FnMut`/`FnOnce`, `move` closures, `Iterator`, `into_iter`/`iter_mut`/`iter`, `map`, `filter`, `collect`, `sum`, `fold`, `zip`, `enumerate`, lazy evaluation | `mod`/multi-file `crate` organization (Chapter 7) |
...
11. Don't import unnecessary `crate`s; prefer the standard library and spellings the book has already shown.

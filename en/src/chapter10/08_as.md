...
Chapter 5 covered `From` and `Into`. They only exist for conversions that are guaranteed not to fail:

```rust,compile_fail
fn main() {
    let x: i32 = 42;
    let y: f64 = f64::from(x); // OK
    let z: i32 = i32::from(3.14_f64); // compile error! not guaranteed safe
}
```
...
- Use `From` / `Into` whenever you can — safety guaranteed at compile time.
...
- `From` / `Into`: only exist for conversions guaranteed to be safe.

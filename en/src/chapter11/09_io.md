# I/O: `stdin` and File Reading/Writing
...
fn main() {
    let content = fs::read_to_string("hello.txt").expect("failed to read");
    println!("{}", content);
}
```
...
fn main() {
    let file = File::open("data.txt").expect("failed to open");
...
fn main() {
    ...
    writeln!(file, "first line").expect("failed to write");
    writeln!(file, "second line").expect("failed to write");
}
```

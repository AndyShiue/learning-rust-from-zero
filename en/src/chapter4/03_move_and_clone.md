...
Recall Episode 1's analogy: `clone` means "replicate the keychain, while making sure it causes no trouble." For `Point`, that means replicating the entire keychain — and that's exactly what the `clone` generated by `#[derive(Clone)]` does: replicate every field. Each variable owns its own data.

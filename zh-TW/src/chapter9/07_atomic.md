...
有些 **atomic 操作**把讀、改、寫合成一個不可分割的動作——其他執行緒不可能看到做到一半的狀態。用 atomic 操作做 `count += 1`，兩個執行緒同時跑，結果一定是 2。
...
- `Ordering::SeqCst`：最嚴格。在這個例子裡，它會保留「寫資料 → 設旗標」和「檢查旗標 → 讀資料」的順序，所以執行緒 B 看到 `ready == true` 時，也一定看得到執行緒 A 先寫好的資料。

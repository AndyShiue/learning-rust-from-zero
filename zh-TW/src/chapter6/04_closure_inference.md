...
2. **如果閉包體需要以可變方式存取捕捉狀態**（例如 `count += 1;`）→ 這個閉包是 **`FnMut`**，可以多次呼叫但需要 `&mut`。
3. **如果閉包體只需要以共享方式存取捕捉狀態**（例如 `println!("{}", name);`）→ 這個閉包是 **`Fn`**，只需要 `&self`。

Rust 會選**能接受最多種使用方式的那個**——如果只需要共享存取，就給 `Fn`（因為 `Fn` 的閉包也能當 `FnMut` 和 `FnOnce` 用）。如果需要可變存取，就變成 `FnMut`。如果有 move，就變成 `FnOnce`。
...
- Rust 根據閉包體的內容自動推斷閉包的種類：move → `FnOnce`、可變存取 → `FnMut`、共享存取 → `Fn`。

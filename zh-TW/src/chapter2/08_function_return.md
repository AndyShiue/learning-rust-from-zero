...
1. `-> i32` 寫在參數後面，告訴 Rust「這個函數會回傳一個 `i32`」。這裡的 `i32` 就是函數的**回傳型別**（return type）。
2. 函數最後一行 `a + b` **沒有加分號** → 這就是回傳值。

`fn add(a: i32, b: i32) -> i32` 這段宣告稱為函數的**簽名**（function signature）。它描述函數的名稱、參數及其型別，以及回傳型別。大括號 `{ ... }` 內的部分則稱為**函數體**（function body），不屬於函數簽名。
...
- 用 `-> 型別` 宣告函數的回傳型別。
- 函數簽名描述函數的名稱、參數及其型別與回傳型別；大括號 `{ ... }` 內的部分是函數體。

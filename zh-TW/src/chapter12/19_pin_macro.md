...
這是一個很有意思的問題。`pin!` 為什麼是巨集，不是一個普通函數？
...
這行不通。`value` 是 `pin` 這個函數自己的區域變數。函數一返回，它的區域變數就被清掉，`value` 跟著消失——於是回傳的那個 `Pin<&mut T>` 立刻變成懸垂參考，指向一塊已經作廢的記憶體。事實上編譯器根本不會讓你回傳一個指向「函數自己區域變數」的參考。

`pin!` 不是普通函數，所以不會有「回傳指向自己 stack 上變數的參考」這個問題。`pin!` 產生的 `Pin<&mut T>` 借用的是使用 `pin!` 的那個作用域裡的值，而不是借用某個普通函數自己的暫時變數。因此，那個借用不會在函數返回時立刻懸垂，你可以安心拿來用。

### 對照 `Box::pin`

那為什麼 `Box::pin` 可以是普通函數？因為它走的是完全不同的路：`Box::pin` 把值放到 **heap** 上，並把那塊 heap 記憶體的所有權包進 `Pin<Box<T>>` 交給你。heap 上擁有所有權的東西活得比「這次函數呼叫」久，函數返回也不會丟掉它，所以回傳一個擁有它的 `Pin<Box<T>>` 完全沒問題。

一句話總結兩者的差別：

- `pin!`：**stack 借用**——在不配置 heap 的情況下，從目前區塊中的值取得 `Pin<&mut T>`；因為普通函數不能回傳指向自己 stack 上變數的參考，所以這件事必須由巨集在呼叫位置完成。
...
- `pin!` 必須是**巨集**而非函數：普通函數不能回傳指向自己 stack 上變數的 `Pin<&mut T>`；函數一返回，那個參考就會懸垂（編譯器也不准你這樣寫）。
- `Box::pin` 交出所有權把值放 heap（活得比這次呼叫久），所以能當普通函數；差別就在「stack 借用 vs heap 擁有」。

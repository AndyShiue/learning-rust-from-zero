...
這份文件定義給 AI 使用的固定題庫，以及出題與批改規則。

請分清楚兩種題目：

1. **固定題庫題目**：本文件後方已寫好的題目。
2. **臨時題目**：符合下方規則時，由 AI 現場出、而不是取自固定題庫的題目。

只有符合下方規則時才可以出臨時題。不能把固定題庫當成靈感，接著擴張成一串非官方、更進階、更複雜、或更像一般 Rust 課程的題目。

## 出題原則

1. 依照 `GUIDE.md` 第 2 節，出題前先確認讀者目前讀到第幾章第幾集。
2. 先找本文件裡讀者進度內的題目，並查看條目狀態。
3. 只有狀態是 `可用` 的條目可以直接當固定題庫題目出；一次只挑 1 個編號題目。同一條目中的多個編號題目是替代選項，不是一組一起出的題目。
4. 狀態是 `不出題` 時，簡短說明條目裡的不出題原因，再建議前面最近的可用題目或請讀者繼續讀。
5. 若沒有合適固定題，或讀者明確要求超前挑戰，依照下方「章節預設策略」處理；該節決定何時可以臨時出 1 題，以及題目前必須怎麼說明。臨時題一定要明說是臨時題，也不能把通用 Rust 題目包裝成固定題庫題目。
6. 臨時題中要求讀者理解或自行完成的部分，預設都必須維持在目前進度內。單純說想練習或想要「挑戰題」，不算同意使用未學內容。
7. **固定句型**以及其中哪些部分可以調整，以 `GUIDE.md` 第 2 節的定義為準；不能把之後才教的內部原理當成練習目標。
8. 符合目前進度的題目若看似需要尚未以固定句型教過的後續語法，應優先重新設計。真的無法避免時，把完整程式碼放在獨立區塊，標題寫成「固定骨架（之後才會教，請勿修改）」。只用一句話說明該區塊的用途，不展開內部原理，並另外清楚指出讀者只需要修改哪一段進度內內容。
9. 只有讀者明確要求超前挑戰時，題目的必要解法才可以使用未學內容。後續內容只能使用眼前所需的最少部分，必須清楚標明之後才會教，不能把它當成讀者應該已經具備的知識，也不能把題目擴張成大型綜合設計。

## 章節預設策略

- 第 1、2 章預設會安排固定練習題。若讀者想要符合目前進度的練習，而且得知指定集數沒有固定題後仍希望針對該集練習，可以臨時出 1 題，但題目前要明說：
...
- 第 3 章或之後的讀者若想要符合目前進度的練習，而且得知沒有固定題庫後仍希望練習，可以臨時出 1 題，但題目前要明說：

> 第 3 章與之後沒有固定題庫。下面是一題依照你目前進度臨時設計的小型挑戰題。

- 第 3 章與之後的臨時題可以整合數個讀者已學概念，稍微提高綜合性。
- 若讀者明確要求超出目前進度的挑戰，可臨時出 1 題，但題目前要明說：

> 這是一題超出你目前進度的臨時挑戰題，可能會用到少量後面才教的內容。我會清楚標出這些內容，你不需要預先知道它們。

## 固定題庫使用規則

1. 只給讀者看選中的編號題目，以及作答所需的讀者可見材料，例如起始程式、預期輸出或執行範例；不要一開始就給「批改重點」「提示方向」「參考答案」。
2. 讀者作答後，用適用於所選編號題目的「批改重點」判斷他是否抓到該題目標。
3. 讀者卡住時，從適用於所選編號題目的「提示方向」一層一層給，不要一次把答案攤開。
4. 讀者要求解答、或已經嘗試過仍卡住時，才可以給該題對應的「參考答案」。
5. 若條目包含多個編號題目，相關的輔助材料區塊與參考答案會依題號順序排列，除非另有明確標示。
6. 固定題庫以「程式題」為主，不為了湊題目放操作確認或名詞問答。
...
- 修完後程式應該印出 `5`。
- 修好後仍會出現「`coins` 被指派的值沒有被讀取」的 warning，這是正常的，不影響執行。
...
- 單獨出題練習價值不高；下一集「跳脫字元」更適合用輸出題練特殊字元。
...
2. 寫一個函數 `buy_ticket(money: i32, price: i32) -> (i32, i32)`，回傳「可以買幾張票」和「剩下多少錢」。在 `main` 裡請使用者輸入手上的錢和正整數票價，最後印出結果。
...
- 第 2 題可以假設票價是正整數；不要加入會讓焦點偏離函數回傳值的輸入驗證。
...
2. 寫一個函數 `print_collatz(n: i32)`，印出輸入數字的 Collatz sequence。規則是：如果 `n` 是 1，就印出 1 並停止；如果 `n` 是偶數，下一個數字是 `n / 2`；如果 `n` 是奇數，下一個數字是 `3 * n + 1`。請在 `main` 裡讀取 `1` 到 `10,000` 的整數，呼叫這個函數，並用遞迴一路印到 1，不要用迴圈。可以假設輸入一定在這個範圍內。
...
- 這一集的重點是練習遞迴，不是研究 Collatz 猜想的數學細節；可以假設輸入是 `1` 到 `10,000`，不需要檢查範圍。

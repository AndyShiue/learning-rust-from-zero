...
`Rc<T>` 和 `Box<T>` 都實作了 `Deref`。像這樣**本職就是替某個內部值當鑰匙**——管理它，並讓 Rust 能透過 `Deref` 觸及它——的型別，我們常稱為**智慧指標（smart pointer）**。其他一些標準庫型別（像 `String` 和 `Vec<T>`）雖然也實作了 `Deref`，但當鑰匙並不是它們的本職；本集聚焦在智慧指標上。

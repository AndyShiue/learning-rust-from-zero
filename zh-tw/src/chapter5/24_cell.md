...
`Cell<T>` 提供內部可變性——它用 `.get()` 複製值、`.set(v)` 替換值，**不需要可變參考**。它不會交出內部值的參考，因此沒有違反借用規則。
...
`Cell<T>` 的 `.get()` 會把值**複製一份**出來（不是借用）。所以要使用 `.get()`，`T` 必須實作 `Copy`。
...
- **要使用 `.get()`，`T` 必須是 `Copy`**——因為 `get` 是複製，不是借用。

...
學會用 `RefCell<T>` 在執行時期檢查借用規則，搭配 `Rc` 實現可變的共享資料。
...
- `RefCell<T>`：用 `.borrow()` 和 `.borrow_mut()` 取得用起來類似共享參考和可變參考的值，`T` **不需要** `Copy`，但**有執行時期成本**（每次借用都要檢查有沒有違反規則）。

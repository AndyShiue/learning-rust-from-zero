...
這份承諾**不只包含 `pub` 的東西，文件記載的行為也可能是其中一部分**。只要這些承諾仍然成立，私有實作細節仍可自由修改。發布或更新專案前最值得問自己的一句話是：「我真的想長期維護這個 `pub` 嗎？」——公開越多，等於承諾越多，日後想改卻不弄壞別人程式的空間就越小。把不必要的東西留成私有（或 `pub(crate)`），是在替未來的自己保留改動的自由。

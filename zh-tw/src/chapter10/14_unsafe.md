...
違反上面任何一條，就叫做**未定義行為**（undefined behavior，簡稱 UB）。它比「結果不確定」嚴重得多：編譯器是**假設這些事永遠不會發生**才做最佳化的，所以一旦發生，壞掉的不只是那一行——整個程式都不再有任何保證，可能算出怪答案、可能當掉，也可能在完全無關的地方出事。
...
- **未定義行為（UB）**：違反了 safe Rust 那些保證。編譯器是假設它永遠不會發生才做最佳化的，所以一旦發生，整個程式都不再有保證。

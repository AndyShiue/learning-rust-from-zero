...
而像之後會學到的 `String` 那樣的型別，它的資料在 heap 上。如果隨便建立副本，就等於建立整個保險箱內容的副本，代價很大。所以 Rust 需要用 move，要建立完整副本就得明確呼叫 `.clone()`。
...
- heap 資料預設 move（只轉移鑰匙）；像 `String` 這類型別的 `clone` 基本上是靠「建立整個保險箱內容的副本」來確保安全。

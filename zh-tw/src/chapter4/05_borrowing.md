...
`&p` 產生的這個東西（也就是 `r`）叫做**參考（reference）**，型別寫成 `&Point`。而「用參考去看別人的資料、但不拿走所有權」這件事，就叫做**借用**。所以借用和參考是一體兩面：**借用**是「借來看」這個動作，**參考**則是這個動作產生的憑證——拿著它就能去看那份資料。之後看到「參考」這個詞，通常指的就是用 `&` 借來的這個值。
...
用 `&` 借用的時候，你**通常只能讀，不能直接修改**。如果你想借來直接修改，那是下一集的內容。

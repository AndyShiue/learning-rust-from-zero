...
### `clone` = 建立鑰匙圈的副本，並確保不會出問題

那如果我真的需要第二份可以用的資料怎麼辦？

Rust 的答案叫做 **`clone`（克隆）**。它的意思是：**建立一份你手上鑰匙圈的副本，並且確保這樣做不會出問題。**

最常見的「確保」方式，就是不去動原本的鑰匙，而是買一個新的保險箱，把內容物建立一份副本放進去，配一把新鑰匙掛在新的鑰匙圈上。這樣兩個人各有自己的保險箱、自己的鑰匙，互不干擾——兩份完全獨立。

不過並非所有型別都用這一招。之後會遇到真的「只多打一把鑰匙」、改用其他機制確保安全的型別。現階段你遇到的 `clone`，都可以理解成「買新保險箱」。
...
- **`clone`（克隆）**：建立鑰匙圈的副本，並確保不會出問題——通常靠「買新保險箱 + 建立內容物副本 + 配新鑰匙」做到，兩份完全獨立。

...
你已經學過 `String`、`&mut` 和 `.expect()`，這段應該都看得懂了。還沒提過的是 `std::io::stdin()` 回傳一個 `Stdin` `struct`，而 `read_line` 回傳的是 `io::Result<usize>`——這是 `Result<usize, io::Error>` 的型別別名，標準庫裡很多輸入輸出函數都用它。

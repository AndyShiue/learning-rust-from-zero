...
理解了「閉包體其實只是一個方法的內容」之後，還可以順便想想一件事：在閉包裡寫 `return` 會發生什麼事？因為閉包體事實上就是某個方法的實作內容，`return` 跳出的是那個方法，也就是**只會跳出最內層的閉包**，而不會跳出包在外面的函數。這其實和 `break` 的預設效果很像——`break` 預設也只會跳出最內層的迴圈，而不是一口氣跳出所有巢狀迴圈。
...
- `Fn`：呼叫時只需要閉包值的共享參考，所以可以重複呼叫。
- 因為閉包體只是一個方法的內容，閉包裡的 `return` 只會跳出最內層的閉包，不會跳出外面的函數——就像 `break` 預設只跳出最內層的迴圈。

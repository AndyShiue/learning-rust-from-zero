...
函數指標 `fn(...) -> ...` 是一個具體的型別，大小固定。但函數沒辦法直接用其他函數裡的區域變數——想用，就得靠參數傳進去。下一集要介紹的閉包（closure）不一樣：閉包**寫在哪裡，旁邊有什麼區域變數就能直接拿來用**，不必一個個當參數傳。
...
- 函數沒辦法直接用其他函數裡的區域變數，要用只能靠參數傳進去；下一集的閉包則可以直接用它所在位置旁邊的區域變數。

...
這個 `Delay` 就是我們接下來好幾集的主角，每當需要一個「要花時間才會好的事件」，我們就拿它來當替身，用它來研究 `.await`、並行、還有 `Waker`。
...
跑起來，你會看到「開始」之後停頓一秒，才印出「一秒過去了」。我們第一個會真正 `Pending` 的 `Future` 成功了！在這一秒之內，`block_on` 的迴圈瘋狂地一直 `poll`、一直拿到 `Pending`，直到時間終於到了才拿到 `Ready`。

### 老實說：這個 `Delay` 被過度簡化了

這個版本能跑，但它其實**偷懶**了——你有沒有注意到 `poll` 裡的 `cx` 參數被寫成 `_cx`，完全沒用到？

`cx` 裡裝著一個叫 `Waker` 的東西，正常的 `Future` 在回 `Pending` 之前，應該用它通知 executor「等我好了再來叫我」。但我們的 `Delay` 完全沒這麼做。為什麼現在還能跑？因為我們搭配的 executor 也一樣笨——它根本不睡覺，回 `Pending` 就馬上再 `poll`，所以就算沒人通知它也無所謂。
...
這個偷懶之後會修。但在那之前，我們得先用這個 `Delay` 把 `.await`、並行這些觀念建立起來。下一集就來看，在 `async` 裡 `.await` 這個 `Delay` 會發生什麼事。

## 重點整理

- `Delay` 用一個計時器模擬「要花時間才會好的事件」：沒到期回 `Pending`，到期回 `Ready`，之後幾集都拿它當替身。
- 真實 I/O 太複雜，所以先用最簡單的計時器來第一次認識 `Pending`。
- 自訂 `Future` 就是 `impl Future`、實作 `poll`，搭配上一集的 `block_on` 就能跑。
- 這個 `Delay` 被過度簡化了：`poll` 沒用到 `cx` 裡的 `Waker`，只因為搭配的 executor 也不睡覺才剛好能跑；換到會睡覺的 executor 就會出問題，後面會修。

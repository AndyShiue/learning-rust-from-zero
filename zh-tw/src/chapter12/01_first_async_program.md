...
不過，`async fn main()` 不能像普通的 `fn main()` 那樣自己直接當程式入口。`#[tokio::main]` 這個 attribute 就是 Tokio 提供的幫手：它會替我們準備 runtime，讓這個 `async fn main()` 可以真的被執行。第 4 集會再把這件事講得更完整；現在先知道這是 Tokio 幫我們啟動 `async` 程式的地方。
...
以 `socket.write_all(response.as_bytes()).await` 為例：把資料寫回瀏覽器可能不會立刻完成，因為網路或對方接收速度都可能讓它需要等一下。這個等待發生在 `tokio::spawn` 出來的背景工作裡；它等待時，runtime 就可以先去推進別的工作，例如讓主迴圈繼續 `accept` 下一個連線，或讓其他連線的處理工作往前跑。

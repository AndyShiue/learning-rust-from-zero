...
順帶釐清它和第 28 集 `watch` 的差別(很容易混):**`Notify` 不帶資料、無狀態;`watch` 帶一個「最新值」、有狀態。** `Notify` 是「我改了那份(放在你自己 `Mutex` 裡的)資料,戳你去看」;`watch` 則是那個要廣播的東西**本身就是一個值**(設定、狀態、shutdown 旗標),它幫你保存最新值、變動時通知,而且**晚來的訂閱者一 `borrow()` 就能讀到當下值**——這點 `Notify` 做不到(它沒存狀態)。一句話:`watch` ≈ 只保留最新一格的廣播 channel,`Notify` ≈ 無資料版的 wake。

...
這就帶出一條鐵律：**一個 `Task` 不能長時間不 `.await`**。如果某個 `Task` 卡著執行緒不放——可能是在做一個很耗時的計算（幾秒的數學運算），也可能是呼叫了某個**同步**的阻塞函式（`std::thread::sleep`、同步讀檔、慢的同步資料庫呼叫）——那它就霸佔了這條執行緒，**同一條執行緒上的其他 `Task` 全都得不到 `poll`**，整個並行就卡死了。

...
`async` 能用少少幾條執行緒推進大量工作，靠的是大家**輪流**。`.await` 是一個**可能**讓出執行緒的位置：如果等待的 `Future` 還沒準備好、回傳 `Pending`，`Task` 就會讓出執行緒，換別人跑；如果 `Future` 已經準備好了，程式會立刻繼續，不會讓出執行緒。

這就帶出一條鐵律：**一個 `Task` 不能長時間不 `.await`**。如果某個 `Task` 卡著 `Thread` 不放——可能是在做一個很耗時的計算（幾秒的數學運算），也可能是呼叫了某個**同步**的阻塞函數（`std::thread::sleep`、同步讀檔、慢的同步資料庫呼叫）——那它就會霸佔這條 `Thread`。在阻塞工作完成前，**這條 `Thread` 無法 `poll` 任何其他 `Task`**。
...
這段計算從頭到尾沒有 `.await`，所以執行它的 `Future` 在計算完成前都無法讓出控制權。如果這類程式碼在透過 `tokio::spawn` 產生的 `Task` 中執行，它會在這段期間佔住一條 worker `Thread`，使該 `Thread` 無法推進其他 `Task`；不過，其他 worker `Thread` 仍可繼續執行。
...
- `tokio::task::spawn_blocking` 把這種工作丟到專用 blocking 池，回傳可 `.await` 的 handle；結果還沒準備好時，你的 `Task` 會讓出執行緒。

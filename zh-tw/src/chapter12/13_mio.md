...
先注意一件事：`mio` 本身**不是** `async`。它不認得 `async fn`，不會幫你建立 `Future`，也不會替你 `.await`。它提供的是比較底層的能力：例如把 socket 設成非阻塞，登記到 `Poll`，然後讓「呼叫 `poll.poll(...)` 的那條 `Thread`」睡著等「socket 是不是 ready 了」。

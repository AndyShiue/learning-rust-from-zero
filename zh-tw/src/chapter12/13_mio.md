...
第 14 集 reactor 會把 `TcpListener`、`TcpStream` 這些 I/O 來源登記給 `Poll`，每個來源各自拿一個 token。到時候流程會像這樣：
...
登記之後，`poll.poll()` 就會睡著，直到**真的有人連進來**——這時 listener 變可讀，`poll` 醒來，`event.token()` 是 `Token(0)`，你就能去 `accept()` 那條連線。這就是 mio 跟 I/O 的關係：**你把 socket 交給它盯著，它替你睡著等，真有事件才叫醒你。**

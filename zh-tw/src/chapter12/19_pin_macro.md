...
這種情況可以用 `std::pin::pin!`。它會在**目前的作用域** 建立一個被釘住的參考，給你一個 `Pin<&mut T>`：
...
所以那個被釘住的值，必須放在**你的程式碼裡**（也就是你呼叫 `pin!` 的地方），才能跟你手上的那個值活得一樣久。
...
巨集就沒有這個問題。巨集會**把程式碼就地貼進你的函式裡**，所以它要釘的那個值，是**你這個函式的區域變數**（壽命就是它的作用域），而不是放在某個一返回就被清掉的別的函式裡。它也沒有「返回」這回事——只是把程式碼貼進來——所以那個借用這個值的 `Pin<&mut T>` 不會懸垂，你可以安心拿來用。
...
- `pin!`：**stack 借用**——值放在你自己的函式裡（當區域變數），交出借用，不配置 heap，但不能傳出作用域，所以必須是巨集。
- `Box::pin`：**heap 擁有**——東西放 heap，交出所有權，可以到處傳，代價是一次 heap 配置。所以可以是普通函數。

## 重點整理

- `pin!` 做 **stack pinning**：在目前的作用域釘住一個值、回傳 `Pin<&mut T>`，不需 heap 配置，適合不必把被釘住的值傳出作用域的情況
- `pin!` 必須是**巨集**而非函式：被釘的值若放進一個函式，函式一返回、它的區域變數就被清掉，回傳的 `Pin<&mut T>` 立刻懸空（編譯器也不准你回傳這種借用）
- 巨集則把程式碼直接貼進你的程式裡，被釘的值跟著你的程式碼一起活著，`Pin<&mut T>` 才不會懸垂；而你拿到的是 `Pin<&mut T>`，不是普通 `&mut T`，所以不能用 `Option::take` 之類的方式把值搬走

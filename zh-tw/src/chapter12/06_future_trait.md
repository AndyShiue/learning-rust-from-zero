...
- `type Output` 是這個 `Future` 完成後會給出的值的型別（這是第 5 章的 associated type）。
- `poll` 是核心方法。它的名字就是「戳一下」的意思——問這個 `Future`：「你好了沒？」
...
所以推進一個 `Future` 的方式就是不斷 `poll` 它：回 `Pending` 就代表還沒好，回 `Ready(value)` 就代表完成、把結果拿走。

### 為什麼 `poll` 的 `self` 是 `Pin<&mut Self>`

你大概注意到一個奇怪的地方：`poll` 的第一個參數不是我們熟悉的 `self` / `&self` / `&mut self`，而是 `self: Pin<&mut Self>`。

先別緊張，`Pin` 的細節這一章後面會花好幾集慢慢講，現在你只要接受一件事：**`Pin` 是一個很特別的型別**。Rust 規定，能放在 `self` 位置的型別只有一小撮「智慧指標類」：

- `self`、`&self`、`&mut self`
- `Box<Self>`、`Rc<Self>`、`Arc<Self>`
- 以及 `Pin<...>`

一般你自訂的型別**不能**這樣用在 `self` 位置。`poll` 之所以能寫成 `self: Pin<&mut Self>`，正是沾了 `Pin` 被列進這份名單的光。目前你可以先把 `Pin<&mut Self>` 想成「一個受了限制的 `&mut Self`」——它讓你能改 `Future` 的內容，但不准你把它整個搬走。為什麼要這個限制，之後會講；現在先往下。
...
`Box::pin(x)` 的型別大致是 `fn pin(x: T) -> Pin<Box<T>>`——把值放到 heap 上，並用 `Pin` 把它釘住。目前你就把它當成「一個受限的指標」就好。

`as_mut` 的型別大致是 `fn as_mut(&mut self) -> Pin<&mut Target>`，對 `Pin<Box<F>>` 來說就是 `-> Pin<&mut F>`，剛好就是 `poll` 需要的 `self: Pin<&mut Self>`。重點是 `as_mut` 只是**借用**，沒有把 `future` 交出去，所以我們的 `loop` 才能拿同一個 `future` 反覆 `poll`。
...
最後提醒一個觀念：Rust 標準庫只定義了 `Future` `trait`，**怎麼實作 executor 完全留給 runtime 自由發揮**。我們這集寫的是「回 `Pending` 就忙著空轉重 poll」的笨版本——超級浪費 CPU。真正的 runtime 會聰明得多：沒事做的時候去睡覺，有事了才被叫醒。
...
- `poll` 的 `self` 是 `Pin<&mut Self>`，因為 `Pin` 是少數能放在 `self` 位置的特別型別；目前先當成「受限的 `&mut Self`」。
- **executor** 負責不斷 `poll` 一個 `Future` 直到 `Ready`；標準庫不附 executor，要自己或靠 runtime 提供。
- `Box::pin` 把值放 heap 並釘住、`as_mut` 借出 `Pin<&mut F>`，兩者配合讓 `loop` 能反覆 `poll` 同一個 `Future`。

...
async fn main() {
    ...
    // write_all：保證把整個 buffer 寫完（內部可能呼叫底層 poll_write 好幾次）
    stream.write_all(b"GET / HTTP/1.0\r\n\r\n").await.expect("寫入失敗");

    // read_exact：保證讀滿 16 個 bytes（內部可能呼叫底層 poll_read 好幾次）
    let mut buf = [0u8; 16];
...
我們說 `read_exact` 和 `write_all` **不是 cancellation safe**：它被中途取消會留下爛攤子（可能已經消費了一部分資料，但整個「填滿 buffer」的動作沒有完成）。所以你**不該**把 `read_exact` 或 `write_all` 這類操作，放進「可能會被中途丟掉」的地方。
